...
export default function OwnerDashboard() {
  ...
  return (
    <OwnerShell title="メインメニュー" subTitle="稼働状況一覧">
      {/* 根本的変更：画面を「左側の操作・サマリー」と「右側のレシート明細」の左右2分割（Grid）に配置 */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 max-w-full mx-auto">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="p-3 bg-white border-2 border-slate-300 rounded">
              <span className="text-[10px] font-black text-slate-400 block uppercase">総稼働（完了）</span>
              <div className="text-sm font-black text-slate-900 mt-2 truncate">{formatTime(stats.totalSeconds)}</div>
            </div>
          </div>

          {/* クイックアクション：イメージ画像右下の「お会計へ」のような、パキッとした超特大の操作ボタン */}
              ...
              {workerStats.map((ws, i) => (
                <div key={i} className="p-3 hover:bg-slate-50 flex items-center justify-between text-xs transition-colors">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">総請負: {ws.total}件</div>
                  </div>
                  <div className="text-right space-y-1 font-medium">
                    <div className="text-blue-600 font-bold">
                      進行: {ws.working}件 <span className="text-slate-400 text-[10px] font-mono">({formatTime(ws.workingSec)})</span>
                    </div>
                    {ws.review > 0 && (
                      <div className="text-rose-600 font-bold bg-rose-50 px-1 rounded inline-block text-[10px]">
                        検収中: {ws.review}件
                      </div>
                    )}
                  </div>
