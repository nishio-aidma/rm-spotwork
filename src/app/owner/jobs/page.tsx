    ...
    <OwnerShell title="案件管理" subTitle="案件の掲載と進捗状況の確認">
      <div className="max-w-6xl mx-auto space-y-6 pb-20 text-slate-800">
        ...
        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">案件一覧 ({jobs.length})</h2>
          ...
          <table className="w-full text-left border-collapse table-auto">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap text-center">募集人数</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {jobs.map((job) => (
                <tr key={job.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  {/* 状態ラベル：下書き（draft）への対応 */}
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-0.5 rounded border text-[9px] font-bold uppercase ${
                      job.status === 'draft' ? 'bg-slate-50 text-slate-400 border-slate-200' :
                      ...
                      job.status === 'completed' ? 'bg-slate-900 text-white border-slate-900' : 
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="text-xs font-bold text-slate-600">
                      {job.workerLimit || 1} <span className="text-[9px] text-slate-300">名</span>
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end items-center gap-4">
                      <Link href={`/owner/jobs/${job.id}`} className="text-[10px] font-bold text-slate-400 hover:text-slate-900 underline underline-offset-4 decoration-slate-200 transition-all">
                        詳細を確認 ≫
                      </Link>
                      <button 
